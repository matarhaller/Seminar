...
def mytargetfunc(x):
	return (0.7**((abs(x**2))))/.6


def answer_hw():
	##question a
	print "rejsampler1d defined"
	...
	n,bins,patches = ax.hist(samples,100,alpha = 0.75,normed = True)
	actual = stats.laplace.pdf(np.linspace(-6,6,100))
	ax.plot(np.linspace(-6,6,100),actual,'r')
	...
	ax.plot(linspace(-6,6,100),actual,'r')
